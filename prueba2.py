...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.combmgrowplants.addItems(["En filas", "En grupos ", "Aleatorio","A lo largo de la parte inferior","500"])
        ...
        buttonCorrer.clicked.connect(self.on_button1_clicked)
        buttonPausa.clicked.connect(self.on_button2_clicked)
        buttonStep.clicked.connect(self.on_button3_clicked)

        # Creamos una gráfica en Matplotlib
        self.plot_example(comedores=[], nacerCom="cerca del centro",plantas=[])

    def update_comedores(self, index):
        num_comedores = int(self.comboTrgetPob.itemText(index))
        nacer_com = self.nacerCom.currentText()
        comedores = self.generate_comedores(num_comedores, nacer_com)
        num_plantas = int(self.combmplants.currentText())
        grow_plants = self.combmgrowplants.currentText()
        plantas = self.generate_plantas(num_plantas, grow_plants)
        self.plot_example(comedores=comedores, nacerCom=nacer_com, plantas=plantas)
    ...
    def plot_example(self, comedores, nacerCom, plantas):
        # Limpiamos la figura
        self.figure.clear()
